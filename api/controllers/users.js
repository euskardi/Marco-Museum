...
module.exports = {
    ...
    addUser: async (req, res, next) => {
        // Notice our app declared app.use(express.json()); for doing this
        const {username, password, usertype, success} = req.body;
        
        const newUser = await UsersService.addUser(username, password, usertype, success)
        res.status(201).json({
            message: "User Added",
            newUser
        });
    ...
    updateUser: async (req, res, next) => {
        ...
        const updateUser = await UsersService.updateUser(username, password, usertype, success)
        res.status(201).json({
